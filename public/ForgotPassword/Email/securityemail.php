...
if (isset($_POST['sendotp'])) {
    ...
    $query = "SELECT * FROM security_person WHERE S_USERNAME='$name' and S_EMAIL='$email'";
    $result = mysqli_query($con, $query);

    if (mysqli_num_rows($result) > 0) {
        $fetch = mysqli_fetch_assoc($result);
        $fusername = $fetch['S_USERNAME'];
        $fpassword = $fetch['S_EMAIL'];
        ...
        if ($name == $fusername && $email == $fpassword) {
            ...
            $mail->SMTPAuth = true;
            $mail->Username = 'johnarancillo110119@gmail.com';
            $mail->Password = 'qhrt jjwl jzsa ghzz';
            $mail->Port = 465;
            $mail->SMTPSecure = 'ssl';
            $mail->isHTML(true);
            ...
            echo "<script type='text/javascript'> document.location = '../security_otp.php'; </script>";
        } else {

            echo "<script type='text/javascript'>alert('Invalid Credentials! Please Try again.');</script>";
            echo "<script type='text/javascript'> document.location = '../security_forgotpassword.php'; </script>";
        }
    } else {
...
if (isset($_POST['verifyotp'])) {
    ...
    if ($otp <> $totp) {
        ...
        echo "<script type='text/javascript'> document.location = '../security_resetpassword.php'; </script>";
    }
}
...
if (isset($_POST['newpass'])) {
    ...
    if ($pass <> $cpass) {
        ...
        echo "<script type='text/javascript'> document.location = '../security_resetpassword.php'; </script>";
            ...
            $query = "UPDATE security_person SET S_PASSWORD='$pass' WHERE S_USERNAME='$mname'";
            $result = mysqli_query($con, $query);
                if ($result) {
